...
// static bool __send_hash_reply(SOCKET socket, bool end, unsigned int key_count,
//                               va_list list) {
//   // bool toret = true;
...
//     buffer[size_kv + 1] = '\n';
//     if (write(socket, buffer, size_kv + 2) == -1) {
//       free(buffer);
...
//   }
//   if (end) {
//     return generic_poll_server_end_hash_reply(socket);
//   }
//   return true;
// }

// inline bool generic_poll_server_start_hash_reply(SOCKET socket) {
//   if (write(socket, "\n", 1) == -1) {
//     generic_poll_server_set_client_state_error(socket);
//     return false;
//   }
...
// inline bool generic_poll_server_end_hash_reply(SOCKET socket) {
//   if (write(socket, "\n", 1) == -1) {
//     generic_poll_server_set_client_state_error(socket);
//     return false;
//   }
...
// bool generic_poll_server_send_hash_reply(SOCKET socket, int key_count, ...) {
//   va_list list;
//   va_start(list, key_count);

//   bool toret = __send_hash_reply(socket, false, key_count, list);
//   va_end(list);
//   return toret;
// }

// bool generic_poll_server_send_full_hash_reply(SOCKET socket, int key_count,
//                                               ...) {
//   va_list list;
//   va_start(list, key_count);
//   bool toret = generic_poll_server_start_hash_reply(socket);
//   if (toret == true)
//     toret = __send_hash_reply(socket, true, key_count, list);
//   va_end(list);
//   s_debug("Full Hash Reply : %d \n", toret);
//   return toret;
// }

// bool generic_poll_server_send_binary_block(SOCKET socket, uint32_t size,
//                                            const char *data) {
...
//   if (write(socket, (const char *)&network_size, 4) == -1) {
//     generic_poll_server_set_client_state_error(socket);
//     return false;
//   }
...
// generic_poll_server_parse_memory_argument(const char **ag, unsigned int ac) {
//   generic_poll_server_memory_argument *toret =
//       (generic_poll_server_memory_argument *)malloc(
//           sizeof(generic_poll_server_memory_argument));
//   toret->next = NULL;
//   toret->offset = 0;
//   toret->size = 0;
...
//       if (i + 1 != ac) {
//         cur->next = (generic_poll_server_memory_argument *)malloc(
//             sizeof(generic_poll_server_memory_argument));
//         cur->next->offset = 0;
//         cur->next->size = 0;
//         cur->next->next = NULL;
...
// void generic_poll_server_free_memory_argument(
//     generic_poll_server_memory_argument *tofree) {
//   if (tofree == NULL)
//     return;
...
// void generic_poll_server_add_callback(generic_poll_server_callback cb,
//                                       void *fntptr) {
//   switch (cb) {
//   case SERVER_STARTED: {
...
// void generic_poll_server_set_client_state_error(SOCKET socket) {
...
// static bool send_error(SOCKET fd, emulator_network_access_error_type type,
...
//   }
//   size_t to_send_size = 1 + strlen("error") + 1 + strlen(error_type) + 1 +
//                         strlen("reason") + 1 + strlen(error_string) + 1 + 2;
...
//   if (write(fd, tosend, towrite) == -1) {
//     free(tosend);
//     generic_poll_server_set_client_state_error(fd);
//     return false;
//   }
//   free(tosend);
//   return true;
...
// static int64_t execute_command(generic_poll_server_client *client,
//                                char *cmd_str, char **args, int args_count) {
//   unsigned int command;
//   bool valid_command = false;
//   unsigned int custom_command_index = 0;
//   for (unsigned int i = 0; i < emulator_network_access_number_of_command; i++) {
//     if (strcmp(cmd_str, emulator_network_access_command_strings[i].string) ==
//         0) {
...
//     if (strncmp(cmd_str, custom_emu_nwa_map[i].string,
//                 strlen(custom_emu_nwa_map[i].string)) == 0) {
//       command = CUSTOM + i;
...
//   if (command >= CUSTOM) {
//     return custom_emu_nwa_map[custom_command_index].function(client->socket_fd,
//                                                              args, args_count);
//   }
//   for (unsigned int i = 0; i < generic_emu_mwa_map_size; i++) {
//     if (generic_emu_mwa_map[i].command == command) {
//       return generic_emu_mwa_map[i].function(client->socket_fd, args,
//                                              args_count);
//     }
//   }
//   send_error(client->socket_fd, invalid_command,
//              "Invalid command or not implemented");
//   return -1;
// }

// static void process_binary_block(generic_poll_server_client *client) {
//   if (client->current_command == bCORE_WRITE) {
//     bool result = generic_poll_server_write_function(
//         client->socket_fd, client->binary_block, client->binary_block_size);
...
//   if (cmd_has_arg) {
//     unsigned int i = 0;
//     // Counting the number of ; to allocate or not memory
//     while (cmd_block[i] != '\n') {
//       if (cmd_block[i] == ';')
//         nb_arg++;
//       i++;
...
//       args = sArgs;
//     } else {
//       args = (char **)malloc((nb_arg + 2) * sizeof(char *));
//       free_args = true;
//     }
...
//         send_error(client->socket_fd, protocol_error,
//                    "Sending invalid binary data block");
//         return false;
//       }
//     }
...
//       {
//         while (read_pos < client->readed_size &&
//                client->binary_header_size < 5) {
//           client->binary_header[client->binary_header_size] =
//               client->readed_data[read_pos];
//           read_pos++;
...
//               ntohl(*((uint32_t *)(client->binary_header + 1)));
//           client->binary_block =
//               (char *)malloc(client->binary_block_defined_size);
//           client->binary_block_size = 0;
//           s_debug("Binary defined block size : %d\n",
//                   client->binary_block_defined_size);
//           if (!(client->binary_block_defined_size ==
//                     client->expected_block_size ||
//                 client->expected_block_size == 0)) {
//             send_error(client->socket_fd, protocol_error,
//                        "Binary block size does not match expected size");
//             return false;
//           }
//         }
//       }
...
//         unsigned int copy_size = MIN(client->readed_size - read_pos,
//                                      client->binary_block_defined_size);
//         memcpy(client->binary_block + client->binary_block_size,
//                client->readed_data + read_pos, copy_size);
...
//       if (client->readed_data[read_pos] == '!' ||
//           isalnum(client->readed_data[read_pos]) != 0) {
//         client->state = GETTING_COMMAND;
//       } else {
//         send_error(client->socket_fd, protocol_error,
//                    "Expecting a command or a signal registration");
//         return false;
//       }
//     }
...
//   int set_result = setsockopt(server_socket, IPPROTO_IPV6, IPV6_V6ONLY,
//                               (char *)&no, sizeof(no));
...
//     else {
//       fprintf(stderr,
//               "Trying to read the port range from NWA_PORT_RANGE variable but "
//               "not a valid port : %s\n",
//               buffer);
//     }
// #if __STDC__ > 201112L
